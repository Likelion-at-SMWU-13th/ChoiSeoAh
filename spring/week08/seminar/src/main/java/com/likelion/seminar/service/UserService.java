...
import static com.likelion.seminar.Exception.HttpStatus.CONFLICT;
import static com.likelion.seminar.Exception.HttpStatus.BAD_REQUEST;
...
public class UserService {
    ...
    public void signup(SignupRequestDTO request) {
        ...
        if (userRepository.existsByUsername(request.getUsername()) {
            throw new CustomException(CONFLICT, "이미 존재하는 아이디입니다.");
        }
        // 이메일 형식 검증
        String email = request.getEmail();
        if (email == null || !email.endsWith("@sookmyung.ac.kr")) {
            throw new CustomException(BAD_REQUEST,"이메일은 @sookmyung.ac.kr로 끝나야 합니다.");
        }
